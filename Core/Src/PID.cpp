...
float PID::set_limits(float _min, float _max) {
  limMin = _min;
  limMax = _max;
}

float PID::set_int_limits(float _intmin, float _intmax) {
  limMaxInt = _intmax;
  limMinInt = _intmin;
}

float PID::set_sampling_time(float _T) { T = _T; }

float PID::set_tau(float _tau) { tau = _tau; }
